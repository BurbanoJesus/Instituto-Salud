<?php 
	...
	if ($_POST['id_integrante'] !== '') {
		...
		$sql = "SELECT menores_a_1.edad_meses_vis,peso,id_visita FROM menores_a_1 JOIN integrantes USING(id_integrante) WHERE id_familia = '$id_familia' AND id_visita <= '$id_visita' AND integrantes.id_integrante = '$id_integrante' ORDER BY id_visita";
		$query = $bd->query($sql);
		// echo $sql;
		// echo '<br>';
		// echo '<br>';
