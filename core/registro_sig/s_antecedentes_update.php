	...
	while (isset($_POST['tb8_1_elemento_'.$cont.''])) {
		...
		if ($tb8_1_elemento == 'si') {
			...
			foreach ($arr_pers as $key => $value) {
				...
				$sql = "SELECT id_integrante FROM integrantes WHERE cod_integrante = '$value'";
				$query = $bd->query($sql); 
				$id_integrante = $query->fetch_row();
				$id_integrante = $id_integrante[0];
				$arr_obse[$key] = (isset($arr_obse[$key])) ? $arr_obse[$key] : 'Null';
				$sql = "SELECT * FROM antecedentes WHERE id_integrante = '$value'";
				$query = $bd->query($sql); 
				$id_integrante = $query->fetch_row();
				if (condition) {
					...
					$sql = "UPDATE antecedentes SET nombre_tabla = '$arr_tablas[1]', nombre_elemento = '$arr_elementos_1[$cont]', edad_inicio = '$arr_edad[$key]', frecuencia = '$arr_frec[$key]', observaciones = '$arr_obse[$key]' WHERE id_integrante = '$id_integrante'";
				}
	...
	while (isset($_POST['tb8_2_elemento_'.$cont.''])) {
		...
		if ($tb8_2_elemento == 'si') {
			...
			foreach ($arr_pers as $key => $value) {
				...
				$sql = "UPDATE antecedentes SET nombre_tabla = '$arr_tablas[2]', nombre_elemento = '$arr_elementos_2[$cont]', edad_inicio = '$arr_edad[$key]',Null, observaciones = '$arr_obse[$key]' WHERE id_integrante = '$id_integrante'";
				echo $sql;
				$query = $bd->query($sql); 
				if (!$query) echo $bd->error;
			}
	...
	while (isset($_POST['tb8_3_elemento_'.$cont.''])) {
		...
		if ($tb8_3_elemento == 'si') {
			...
			foreach ($arr_pers as $key => $value) {
				...
				$sql = "UPDATE antecedentes SET nombre_tabla = '$arr_tablas[3]', nombre_elemento = '$arr_elementos_3[$cont]', edad_inicio = '$arr_edad[$key]', frecuencia = '$arr_frec[$key]', observaciones = '$arr_obse[$key]' WHERE id_integrante = '$id_integrante'";
				echo $sql;
				$query = $bd->query($sql); 
				if (!$query) echo $bd->error;
			}
	...
	while (isset($_POST['tb8_4_elemento_'.$cont.''])) {
		...
		if ($tb8_4_elemento == 'si') {
			...
			foreach ($arr_pers as $key => $value) {
				...
				$sql = "UPDATE antecedentes SET nombre_tabla = '$arr_tablas[4]', nombre_elemento = '$arr_elementos_4[$cont]', edad_inicio = '$arr_edad[$key]', frecuencia = '$arr_frec[$key]', observaciones = '$arr_obse[$key]' WHERE id_integrante = '$id_integrante'";
				echo $sql;
				$query = $bd->query($sql); 
				if (!$query) echo $bd->error;
			}
	...
	while (isset($_POST['tb8_5_elemento_'.$cont.''])) {
		...
		if ($tb8_5_elemento == 'si') {
			...
			foreach ($arr_pers as $key => $value) {
				...
				$sql = "UPDATE antecedentes SET nombre_tabla = '$arr_tablas[5]', nombre_elemento = '$arr_elementos_5[$cont]', edad_inicio = '$arr_edad[$key]',Null, observaciones = '$arr_obse[$key]' WHERE id_integrante = '$id_integrante'";
				echo $sql;
				$query = $bd->query($sql); 
				if (!$query) echo $bd->error;
			}
