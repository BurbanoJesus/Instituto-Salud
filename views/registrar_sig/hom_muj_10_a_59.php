...
<body>
	<main>
		...
		$cont = 1;
		$sql = "SELECT * FROM integrantes JOIN de_10_a_59 USING(id_integrante)  WHERE integrantes.id_familia = '$id_familia' AND edad_anos >= 10 AND edad_anos < 60 AND id_visita = (SELECT MAX(id_visita)-1 FROM visitas WHERE id_familia = '$id_familia')";
		$query = $bd->query($sql);
		$query_2 = $query; 
		...
		if ($query->num_rows == 0 or $query_s->num_rows > 0) {
			...
			header("location: /instituto/views/registrar_sig/adultos_60_mas.php");
			return False;
		}
		else{ 
			
		?>
		<form id="form7" action="/instituto/core/registro_sig/s_hom_muj_10_a_59.php" method="POST">
			<div class="divh1">
					...
					<table id="tb5_hombresmujeres_10a59">
						...
						<tbody>
							<?php  while ( $row = $query->fetch_object()) {
								$collect[] = $row;
								// var_dump($row);
								$edad = $row->edad_actual_a;
							?>
							<tr>
								<td><?php echo $cont?></td>
								<td><input class="tb5_nombres" type="text" name="tb5_nombres_<?php echo $cont?>" value="<?php echo $row->nombres?>"></td>
								<td><input type="number" name="tb5_edad_<?php echo $cont?>" step=0.1  value="<?php echo $edad?>"></td>
								<?php 
								 ...
								 ?>
								<td><input type="number" name="tb5_metodo_<?php echo $cont?>" step=0.1 min=0 max= 10 value="<?php echo $row->metodo_planificacion?>"></td>
								...
								?>
								<td class="br_verde2"><input type="number" name="tb5_citologia_d_<?php echo $cont?>" step=0.1  value="<?php echo $row->citologia_d;?>" value="" min=1 max=31></td>
								<td class="br_verde2"><input type="number" name="tb5_citologia_m_<?php echo $cont?>" step=0.1  value="<?php echo $row->citologia_m;?>" value="" min=1 max=12></td>
								...
								<td class="br_verde"><input type="number" name="tb5_examen_prostat_fecha_d_<?php echo $cont?>" step=0.1  value="<?php echo $row->examen_pros_d?>" value="" min=1 max=31></td>
								<td class="br_verde"><input type="number" name="tb5_examen_prostat_fecha_m_<?php echo $cont?>" step=0.1  value="<?php echo $row->examen_pros_m?>" value="" min=1 max=12></td>
